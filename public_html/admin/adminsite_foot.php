...
if($typeid!=0){
	$ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'site_foot WHERE stpID='.$typeid;
	$sql = 'SELECT siteSort,siteID,siteName,siteUrl,siteStatus,siteColor FROM '.DBPREFIX.'site_foot WHERE stpID='.$typeid;
}else{
	$ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'site_foot WHERE 1=1';
	$sql = 'SELECT siteSort,siteID,siteName,siteUrl,siteStatus,siteColor FROM '.DBPREFIX.'site_foot WHERE 1=1';
}
	...
	if($recordCount = $objC -> GetOne($ct)){
		...
		$typeSql = "SELECT * FROM ".DBPREFIX."type_foot ORDER BY stpSort ASC";
...
$objS -> display("admin/adminsite_foot.tpl");
?>
