    ...
    public function deleteAction()
    {
		if($_GET){
			...
			foreach($siteIDArr as $sid){
				$sql = "delete from ".DBPREFIX."site_foot where siteID = '".$sid."'";
    ...
    public function deletetypeAction()
    {
		if($_GET){
			...
			foreach($stpIDArr as $sid){
				$sql = "delete from ".DBPREFIX."type_foot where stpID = '".$sid."'";
				$rs = $this->objC->RunQuery($sql);
			}
			if(!$rs){
				$result = array('flag'=>'error');
    ...
    public function editAction()
    {
    	...
    	$siteIDArr = isset($req['siteID'])?$req['siteID']:array();
			...
			foreach($siteIDArr as $key => $sid){
				...
				if($sid){													//update
					$sql = "update ".DBPREFIX."site_foot set siteName='".unescape($siteNameArr[$key])."',siteUrl='".unescape(htmlspecialchars_decode($siteUrlArr[$key]))."',siteStatus='".unescape($siteStatusArr[$key])."',cDate='".time()."',siteSort='".unescape($siteSortArr[$key])."',siteColor='".unescape($siteColorArr[$key])."' WHERE siteID={$sid}";
					$rs = $this->objC->RunQuery($sql);
					if($rs)
					...
					$sql = "insert into ".DBPREFIX."site_foot (siteName,siteUrl,siteStatus,cDate,siteSort,siteCreateDate,stpID,siteColor) values('".unescape($siteNameArr[$key])."','".unescape($siteUrlArr[$key])."','".unescape($siteStatusArr[$key])."','".time()."','".unescape($siteSortArr[$key])."',".time().",'".unescape($siteStpIDArr[$key])."','".unescape($siteColorArr[$key])."')";
					$rs = $this->objC->RunQuery($sql);
					if($rs){
    ...
    public function edittypeAction()
    {
    	if($_GET){
			...
			foreach($stpIDArr as $key => $sid){
				...
				if($sid){													//update
					$sql = "update ".DBPREFIX."type_foot set stpName='".unescape($stpNameArr[$key])."',stpHtmlName='".unescape($stpHtmlNameArr[$key])."',stpSort='".unescape($stpSortArr[$key])."' WHERE stpID={$sid}";
					$rs = $this->objC->RunQuery($sql);
					if($rs)
					...
					$sql = "insert into ".DBPREFIX."type_foot (stpName,stpHtmlName,stpSort,stpCreateDate,stpParentID,stpShortName) values('".unescape($stpNameArr[$key])."','".unescape($stpHtmlNameArr[$key])."','".unescape($stpSortArr[$key])."',".time().",".$stpParentID.",'')";
					$rs = $this->objC->RunQuery($sql);
					if($rs){
						$sid = $this->objC->GetInsertId();
