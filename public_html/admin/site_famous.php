    ...
    public function deleteAction()
    {
		if($_GET){
			...
			foreach($siteIDArr as $sid){
				$sql = "delete from ".DBPREFIX."site_famous where siteID = '".$sid."'";
    ...
    public function editAction()
    {
    	if($_GET){
			...
			foreach($siteIDArr as $key => $sid){
				...
				if($sid){													//update
					$sql = "update ".DBPREFIX."site_famous set siteName='".unescape($siteNameArr[$key])."',siteUrl='".unescape(htmlspecialchars_decode($siteUrlArr[$key]))."',siteStatus='".unescape($siteStatusArr[$key])."',cDate='".time()."',siteSort='".unescape($siteSortArr[$key])."',siteColor='".unescape($siteColorArr[$key])."' WHERE siteID={$sid}";
					$rs = $this->objC->RunQuery($sql);
					if($rs)
					...
					$sql = "insert into ".DBPREFIX."site_famous(siteName,siteUrl,siteStatus,cDate,siteSort,siteColor) values('".unescape($siteNameArr[$key])."','".unescape($siteUrlArr[$key])."','".unescape($siteStatusArr[$key])."','".time()."','".unescape($siteSortArr[$key])."','".unescape($siteColorArr[$key])."')";
					$rs = $this->objC->RunQuery($sql);
					if($rs){
