...
if ($act== 'update'){
	...
	if(isset($_POST['upAd'])){
		...
		if($objC -> GetAffectedRows() < 0){
			AlertMsg("操作失败！","adminAd_6.php");
		}else{
			AlertMsg("操作成功！","adminAd_6.php");
		}
    }
}else if($act == 'add'){
	if($_POST['addAd']){
    ...
    $ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'admin_ad WHERE 1=1 and id="index06"';
    $sql = 'SELECT * FROM '.DBPREFIX.'admin_ad WHERE 1=1 and id="index06" order by orders ASC';
...
$adImgPath = '../static/images/ad6.jpg';
