...
if($typeid!=0){
	$ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'site_cool WHERE stpID='.$typeid;
	$sql = 'SELECT siteSort,siteID,siteName,siteUrl,siteStatus,siteColor FROM '.DBPREFIX.'site_cool WHERE stpID='.$typeid;
}else{
	$ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'site_cool WHERE 1=1';
	$sql = 'SELECT siteSort,siteID,siteName,siteUrl,siteStatus,siteColor FROM '.DBPREFIX.'site_cool WHERE 1=1';
}
	...
	if($recordCount = $objC -> GetOne($ct)){
		...
		$typeSql = "SELECT * FROM ".DBPREFIX."type_cool ORDER BY stpSort ASC";
...
$objS -> display("admin/adminsite_cool.tpl");
?>
