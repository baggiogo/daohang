...
if ($act== 'update'){
	...
	if(isset($_POST['upAd'])){
		...
		if($objC -> GetAffectedRows() < 0){
			AlertMsg("操作失败！","adminAd_3.php");
		}else{
			AlertMsg("操作成功！","adminAd_3.php");
		}
    }
}else if($act == 'add'){
	if($_POST['addAd']){
    ...
    $ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'admin_ad WHERE 1=1 and id="index03"';
    $sql = 'SELECT * FROM '.DBPREFIX.'admin_ad WHERE 1=1 and id="index03" order by orders ASC';
...
$adImgPath = '../static/images/ad3.jpg';
