...
if ($act== 'update'){
	...
	if(isset($_POST['upAd'])){
		...
		if($objC -> GetAffectedRows() < 0){
			AlertMsg("操作失败！","adminAd_2.php");
		}else{
			AlertMsg("操作成功！ 注意：请到 [生成静态] 栏目下重新生成首页!","adminAd_2.php");
		}
    }
}else if($act == 'add'){
	if($_POST['addAd']){
    ...
    $ct = 'SELECT count(*) as ct FROM '.DBPREFIX.'admin_ad WHERE 1=1 and id="index02"';
    $sql = 'SELECT * FROM '.DBPREFIX.'admin_ad WHERE 1=1 and id="index02" order by orders ASC';
...
$adImgPath = '../static/images/ad2.jpg';
