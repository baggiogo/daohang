...
class adminSiteController extends abstractController
{
    public function deleteAction()
    {
		if($_GET){
			...
			foreach($siteIDArr as $sid){
				$sql = "delete from ".DBPREFIX."site_cool where siteID = '".$sid."'";
    ...
    public function deletetypeAction()
    {
		if($_GET){
			...
			foreach($stpIDArr as $sid){
				$sql = "delete from ".DBPREFIX."type_cool where stpID = '".$sid."'";
				$rs = $this->objC->RunQuery($sql);
			}
			if(!$rs){
				$result = array('flag'=>'error');
    ...
    public function editAction()
    {
    	if($_GET){
			...
			foreach($siteIDArr as $key => $sid){
				...
				if($sid){													//update
					$sql = "update ".DBPREFIX."site_cool set siteName='".unescape($siteNameArr[$key])."',siteUrl='".unescape(htmlspecialchars_decode($siteUrlArr[$key]))."',siteStatus='".unescape($siteStatusArr[$key])."',cDate='".time()."',siteSort='".unescape($siteSortArr[$key])."',siteColor='".unescape($siteColorArr[$key])."' WHERE siteID={$sid}";
					$rs = $this->objC->RunQuery($sql);
					if($rs)
					...
					$sql = "insert into ".DBPREFIX."site_cool (siteName,siteUrl,siteStatus,cDate,siteSort,siteCreateDate,stpID,siteColor) values('".unescape($siteNameArr[$key])."','".unescape($siteUrlArr[$key])."','".unescape($siteStatusArr[$key])."','".time()."','".unescape($siteSortArr[$key])."',".time().",'".unescape($siteStpIDArr[$key])."','".unescape($siteColorArr[$key])."')";
					$rs = $this->objC->RunQuery($sql);
					if($rs){
    ...
    public function edittypeAction()
    {
    	if($_GET){
			...
			$stpNameArr = isset($req['stpName'])?$req['stpName']:'';
			$stpHtmlNameArr = isset($req['stpHtmlName'])?$req['stpHtmlName']:'';
			$stpSortArr = isset($req['stpSort'])?$req['stpSort']:'';
			$return = array();
			foreach($stpIDArr as $key => $sid){

				if($sid){													//update
					$sql = "update ".DBPREFIX."type_cool set stpName='".unescape($stpNameArr[$key])."',stpHtmlName='".unescape($stpHtmlNameArr[$key])."',stpSort='".unescape($stpSortArr[$key])."' WHERE stpID={$sid}";
					$rs = $this->objC->RunQuery($sql);
					if($rs)
					...
					$sql = "insert into ".DBPREFIX."type_cool (stpName,stpHtmlName,stpSort,stpCreateDate,stpParentID,stpShortName) values('".unescape($stpNameArr[$key])."','".unescape($stpHtmlNameArr[$key])."','".unescape($stpSortArr[$key])."',".time().",".$stpParentID.",'')";
					$rs = $this->objC->RunQuery($sql);
					if($rs){
						$sid = $this->objC->GetInsertId();
