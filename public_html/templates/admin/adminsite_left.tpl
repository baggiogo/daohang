...
siteUrl = urlf(siteUrl);
	...
	$.getJSON("site_left.php?a=edit&siteID[]="+escape(siteID)+"&siteColor[]="+escape(siteColor)+"&siteSort[]="+escape(siteSort)+"&siteName[]="+escape(siteName)+"&siteUrl[]="+escape(siteUrl)+"&siteStatus[]="+escape(siteStatus)+"&stpID[]=<?{$typeid}?>", function(json){
...
function doTypeLineEdit(rid,nid){
	...
	$.getJSON("site_left.php?a=edittype&stpID[]="+escape(stpID)+"&stpSort[]="+escape(stpSort)+"&stpName[]="+escape(stpName)+"&stpHtmlName[]="+escape(stpHtmlName)+"&stpStatus[]="+escape(stpStatus)+"&stpParentID=<?{$typeid}?>", function(json){
...
function submitTypeAllEdit(){
	...
	$("#typedatatable").find("tr").each(function(){
		...
		if(trid.substr(0,13)=='type_new_row_'){
			...
			url += "&stpID[]="+escape(stpID)+"&stpSort[]="+escape(stpSort)+"&stpName[]="+escape(stpName)+"&stpHtmlName[]="+escape(stpHtmlName)+"&stpStatus[]="+escape(stpStatus)+"&stpParentID=<?{$typeid}?>";
    ...
    <td height="52" bgcolor="#FFFFFF"><h1>页脚导航管理</h1>
      <div class="search">
        ...
        <select name="sStatus">
          ...
          <option value="1" <?{if $smarty.get.sStatus == '1'}?>selected="selected"<?{/if}?>>显示</option>
          <option value="0" <?{if $smarty.get.sStatus == '0'}?>selected="selected"<?{/if}?>>不显示</option>
        </select>
		...
		<table id="datatable" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" title="双击编辑">
          ...
          <tr id="edit_row_<?{$arrSite[loop].siteID}?>" style="display:none">
            ...
            <select name="siteStatus[]" id="siteStatus_<?{$arrSite[loop].siteID}?>">
            	<option value="1" <?{if $arrSite[loop].siteStatus == 1}?>selected<?{/if}?>>显示</option>
      ...
      <tr>
		...
		<table id="typedatatable" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" title="双击编辑">
          ...
          <tr id="type_display_row_<?{$allType[loop].stpID}?>" ondblclick="submitTypeLineEdit(<?{$allType[loop].stpID}?>)">
            ...
            <td class="tr_a"><?{if $allType[loop].stpStatus == 1}?>
              显示
              <?{else}?>
              <font color="#FF0000">不显示</font>
              <?{/if}?></td>
            <td class="tr_a"><a href="javascript:;" onclick="submitTypeLineEdit(<?{$allType[loop].stpID}?>)"><img src="images/ico_edit.gif" title="编辑"></a><a onclick="deleteTypeLineSite(<?{$allType[loop].stpID}?>)"><img src="images/ico_del.gif" title="删除"></a><a href="./adminsite_left.php?typeid=<?{$allType[loop].stpID}?>"><img src="images/ico_set.gif" title="下一级"></a></td>
          ...
          <tr id="type_edit_row_<?{$allType[loop].stpID}?>" style="display:none">
            ...
            <td class="tr_a">
            <select name="stpStatus[]" id="stpStatus_<?{$allType[loop].stpID}?>">
            	<option value="1" <?{if $allType[loop].stpStatus == 1}?>selected<?{/if}?>>显示</option>
            	<option value="0">不显示</option>
			</select>
            </td>
            <td class="tr_a"><a  href="javascript:;" onclick="doTypeLineEdit(<?{$allType[loop].stpID}?>)"><img src="images/ico_ok.gif" /></a><a  href="javascript:;" onclick="hideTypeLineEdit(<?{$allType[loop].stpID}?>)"><img src="images/ico_cancel.gif" /></a><a onclick="deleteTypeLineSite(<?{$allType[loop].stpID}?>)"><img src="images/ico_del.gif" title="删除"></a><a href="./adminsite_left.php?typeid=<?{$allType[loop].stpID}?>"><img src="images/ico_set.gif" title="下一级"></a></td>
          </tr>
          <?{/section}?>
      </table>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" style="border-bottom:none">
