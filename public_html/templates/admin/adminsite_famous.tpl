...
siteUrl = urlf(siteUrl);
	...
	$.getJSON("site_famous.php?a=edit&siteID[]="+escape(siteID)+"&siteColor[]="+escape(siteColor)+"&siteSort[]="+escape(siteSort)+"&siteName[]="+escape(siteName)+"&siteUrl[]="+escape(siteUrl)+"&siteStatus[]="+escape(siteStatus), function(json){
...
siteUrl = urlf(siteUrl);
	...
	if(url!=''){
		url = 'site_famous.php?a=edit'+url;
...
function deleteLineSite(id,trid){
	...
	if(id!=''){
		...
		$.getJSON("site_famous.php?a=delete&siteID[]="+escape(siteID), function(json){
			if(json.flag=='delete'){
...
<div id="box">
  ...
  <tr>
    <td height="52" bgcolor="#FFFFFF"><h1>名站管理</h1>
      <div class="search">
	...
	<table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" style="border-bottom:none">
      <tr>
        ...
        <div style="position:relative; float:right"><img src="images/help.gif" onclick="showhelp('adminsite_famous',1);" title="点击查看帮助" style="cursor:pointer"/>
        <div id="help" style="display:none;height:auto;width:220px;position:absolute;top:20px;left:-32px;z-index:201;">
