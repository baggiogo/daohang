...
siteUrl = urlf(siteUrl);
	...
	$.getJSON("site_cool.php?a=edit&siteID[]="+escape(siteID)+"&siteColor[]="+escape(siteColor)+"&siteSort[]="+escape(siteSort)+"&siteName[]="+escape(siteName)+"&siteUrl[]="+escape(siteUrl)+"&siteStatus[]="+escape(siteStatus)+"&stpID[]=<?{$typeid}?>", function(json){
...
function doTypeLineEdit(rid,nid){
	...
	$.getJSON("site_cool.php?a=edittype&stpID[]="+escape(stpID)+"&stpSort[]="+escape(stpSort)+"&stpName[]="+escape(stpName)+"&stpHtmlName[]="+escape(stpHtmlName)+"&stpParentID=<?{$typeid}?>", function(json){
...
function editTypeResult(data,upid){
	if(data.flag == 'insert'){
			...
			var data = new Array(data.stpSort,data.stpName,data.stpHtmlName,'<a href="javascript:;" onclick="submitTypeLineEdit('+data.stpID+')"><img src="images/ico_edit.gif" title="编辑"></a><a onclick="deleteTypeLineSite('+data.stpID+')"><img src="images/ico_del.gif" title="删除"></a><a href="./adminsite_cool.php?typeid='+data.stpID+'"><img src="images/ico_set.gif" title="下一级"></a>');
			i=0;
			tr.find("td").each(function(){
				$(this).html(data[i++]);
			})
...
function submitTypeAllEdit(){
	...
	if(url!=''){
		url = 'site_cool.php?a=edittype'+url;
		$.getJSON(url,function(json){
			for(key in json){
...
function deleteLineSite(id,trid){
	...
	if(id!=''){
		...
		$.getJSON("site_cool.php?a=delete&siteID[]="+escape(siteID), function(json){
			if(json.flag=='delete'){
...
function deleteTypeLineSite(id,trid){
	...
	if(id!=''){
		// 删除真数据
		$.getJSON("site_cool.php?a=deletetype&stpID[]="+escape(id), function(json){
			if(json.flag=='delete'){
      ...
      <tr>
		...
		<table id="typedatatable" width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" title="双击编辑">
          ...
          <tr id="type_title">
            ...
            <td width="30%"  class="tr1" id="sort_stpHtmlName">链接地址</td>
            <td width="12%"  class="tr1">操作</td>
          </tr>
          ...
          <tr id="type_display_row_<?{$allType[loop].stpID}?>" ondblclick="submitTypeLineEdit(<?{$allType[loop].stpID}?>)">
            ...
            <td class="tr_a"><a href="javascript:;" onclick="submitTypeLineEdit(<?{$allType[loop].stpID}?>)"><img src="images/ico_edit.gif" title="编辑"></a><a onclick="deleteTypeLineSite(<?{$allType[loop].stpID}?>)"><img src="images/ico_del.gif" title="删除"></a><a href="./adminsite_cool.php?typeid=<?{$allType[loop].stpID}?>"><img src="images/ico_set.gif" title="下一级"></a></td>
          ...
          <tr id="type_edit_row_<?{$allType[loop].stpID}?>" style="display:none">
            ...
            <td class="tr_a"><a  href="javascript:;" onclick="doTypeLineEdit(<?{$allType[loop].stpID}?>)"><img src="images/ico_ok.gif"></a><a  href="javascript:;" onclick="hideTypeLineEdit(<?{$allType[loop].stpID}?>)"><img src="images/ico_cancel.gif"></a><a onclick="deleteTypeLineSite(<?{$allType[loop].stpID}?>)"><img src="images/ico_del.gif" title="删除"></a><a href="./adminsite_cool.php?typeid=<?{$allType[loop].stpID}?>"><img src="images/ico_set.gif" title="下一级"></a></td>
          </tr>
          <?{/section}?>
      </table>
      <table width="100%" border="0" cellspacing="0" cellpadding="0" class="table_list" style="border-bottom:none">
