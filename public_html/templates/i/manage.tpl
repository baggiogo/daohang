...
<iframe width="470" height="22" frameborder="0" scrolling="no" allowtransparency="true" src="http://<?{$smarty.const.SITE_DOMAIN}?>/data/html/tianqi.htm"></iframe>
  ...
  <div class="r03"></div>
  <div class="r02"><a href="<?{if $sets == 1}?>http://<?{$smarty.const.SITE_DOMAIN}?>/i/?<?{$userdomain}?><?{elseif $sets == 2}?>http://<?{$smarty.const.SITE_DOMAIN}?>/i/<?{$userdomain}?><?{else}?>http://<?{$userdomain}?>.<?{$site_domain}?><?{/if}?>" ><font color="#FF0000">返回首页</font></a>&nbsp;&nbsp;|&nbsp;&nbsp;
 <a href="http://<?{$smarty.const.SITE_DOMAIN}?>/i/index.php?c=login&act=logout">注销</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="http://bbs.5w.com/thread-113-1-1.html" target="_blank">帮助</a>&nbsp;&nbsp;|&nbsp;&nbsp;<a href="http://<?{$smarty.const.SITE_DOMAIN}?>/" target="_blank">返回网站首页</a></div>
  <div class="r01"></div>
</div>
  ...
  <div class="title">
   <span class="<?{if $setdomain == 0}?>tab2<?{else}?>tab<?{/if}?>" id="t1" onclick="changeset(1);">首页设置</span> <span id="t2" class="<?{if $setdomain == 0}?>tab<?{else}?>tab2<?{/if}?>" onclick="changeset(2);">个性化域名</span><span id="t3" class="tab2" onclick="changeset(3);">修改密码</span></div>
	...
	<form action="http://<?{$smarty.const.SITE_DOMAIN}?>/i/?c=manage&a=change" method="post">
	    <div class="mata">
	首页个性标题：<input type="text" name="title" value="<?{$title|default:'5W个性导航'}?>" maxlength="80" size="30" style="height:23px; line-height:23px;" /> 修改属于自己的导航标题
	</div>
...
<div id="divUrl" title="<font color=black>添加网址</font>" style="display:none; padding-top:20px;">
	<form id="addUrlForm" method="post" action="#" style="margin:0; padding:0;" onSubmit="return false;">
		<table style="width:350px; margin:0 auto; padding-left:5px;" border="0" cellpadding="0" cellspacing="0">
			<tr>
				<td align="right" height="30" width="70">标题：</td><td align="left"><input type="text" style="width:150px; height:15px;" maxlength="10" id="UrlName" name="UrlName" /> *最多可填写6个字</td>
			</tr>
			<tr>
