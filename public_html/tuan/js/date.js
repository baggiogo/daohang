...
{	
    ...
    var ey = date.getFullYear()+25;  //最大值 → 50 年后
...
Calendar.prototype.draw = function() {
  ...
  mvAry[mvAry.length]  = '      <tr>';
  for(var i = 0; i < 7; i++) {
    mvAry[mvAry.length]  = '      <th style="line-height:18px;font-weight:normal;background-color:' + calendar.colors["tr_bg"] + ';color:' + calendar.colors["tr_word"] + ';">' + Calendar.language["weeks"][this.lang][i] + '</th>';
  }
  mvAry[mvAry.length]  = '      </tr>';
  for(var i = 0; i < 6;i++){
    ...
    for(var j = 0; j < 7; j++) {
      if (j == 0){
        mvAry[mvAry.length]  = '  <td style="line-height:18px;cursor:default;color:' + calendar.colors["sun_word"] + ';"></td>';
      } else if(j == 6) {
        mvAry[mvAry.length]  = '  <td style="line-height:18px;cursor:default;color:' + calendar.colors["sat_word"] + ';"></td>';
      } else {
        mvAry[mvAry.length]  = '  <td style="line-height:18px;cursor:default;"></td>';
      }
    }
    mvAry[mvAry.length]  = '    </tr>';
  }
...
Calendar.prototype.bindData = function () {
  ...
  var dateArray = this.getMonthViewArray(this.date.getFullYear(), this.date.getMonth());//2008-06-19 由xugang9804@gmail.com修改,getYear ff不兼容
      ...
      tds[i].onclick = function () {
        if (calendar.dateControl != null){
          //calendar.dateControl.value = '';
		  	//往期回顾
			try{			  
				index_review(new Date(calendar.date.getFullYear(),calendar.date.getMonth(),this.innerHTML).format(calendar.dateFormatStyle));
			}catch(err){
...
document.onclick = commoncallfunc;
function commoncallfunc(){
	return
	//if(event.srcElement.getAttribute("functionname"))
	//	SelectDate(event.srcElement,'yyyy-MM-dd');
}
